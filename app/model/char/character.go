...
type Characters struct {
	...
	Inventory            CharacterInventorySlice `json:"inventory,omitempty"`
	Stats                CharacterStatsSlice     `json:"stats"`
...
type DBCharacters struct {
	...
	Inventory            DBCharacterInventorySlice `bun:"rel:has-many,join:guid=guid"`
	Stats                DBCharacterStatsSlice     `bun:"rel:has-many,join:guid=guid"`
...
func (entry *DBCharacters) ToWeb() *Characters {
	...
	return &Characters{
		...
		Inventory:            entry.Inventory.ToWeb(),
		Stats:                entry.Stats.ToWeb(),
	}
}
